...
#define TLOG(logger, lvl, tag, message, ...)                                   \
    do {                                                                       \
...
#else
#define TLOG_T(message, ...)                                                   \
    TLOG(tlab::log::logger::instance(), loglvl::trace, "", message,            \
         ##__VA_ARGS__)
#define TLOG_D(message, ...)                                                   \
    TLOG(tlab::log::logger::instance(), loglvl::debug, "", message,            \
         ##__VA_ARGS__)
#define TLOG_I(message, ...)                                                   \
    TLOG(tlab::log::logger::instance(), loglvl::info, "", message,             \
         ##__VA_ARGS__)
#define TLOG_W(message, ...)                                                   \
    TLOG(tlab::log::logger::instance(), loglvl::warn, "", message,             \
         ##__VA_ARGS__)
#define TLOG_E(message, ...)                                                   \
    TLOG(tlab::log::logger::instance(), loglvl::error, "", message,            \
         ##__VA_ARGS__)
#endif
#endif
